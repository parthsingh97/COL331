...
scheduler(void)
{
  ...
  for(;;){
    ...
    // TODO: priority schedular here chooe the highest priority 
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      for(p_itr = ptable.proc; p_itr < &ptable.proc[NPROC]; p_itr++){
        if(p_itr->state != RUNNABLE)
          continue;
        if (max_p->priority < p_itr->priority)
        {
          max_p = p_itr;
        }        
      }
      ...
      // // Update counter and priority of each process
      // acquire(&ptable.lock);
      for(p_itr = ptable.proc; p_itr < &ptable.proc[NPROC]; p_itr++){
        if(p_itr->state != RUNNABLE)
          continue;
